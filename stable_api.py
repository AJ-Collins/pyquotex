...
class Quotex(object):
    ...
    async def get_balance(self):
        while self.api.account_balance is None:
            await asyncio.sleep(0.5)
        print(f"API Response (Account Balance): {self.api.account_balance}")
        print(f"Account Type: {self.api.account_type}")
        if self.api.account_balance:
            balance = self.api.account_balance.get("demoBalance") \
            if self.api.account_type > 0 else self.api.account_balance.get("liveBalance")
        else:
            balance = 0.0 

        # If balance is None, fallback to 0.0
        balance = 0.0 if balance is None else balance

        # Calculate and return the balance with profit
